...
class DoorState(ToplevelChannelState, DTMFHandler):
    ...
    async def player(self):
        ...
        if f is False:
            ...
            p = await self.channel.playWithId(
                playbackId=playback_id, media="tone:beep;tonezone=de"
            )
            await anyio.sleep(0.7)
            try:
                await p.stop()
            except asyncari.util.BadStatus:
                pass
        else:
            if f is None:
    ...
    async def on_start(self):
        ...
        async with with_bridge(self.obj) as br:
            ...
            async def sub():
                ...
                except asyncari.util.BadStatus as exc:
...
async def _run_bridge(obj, *, task_status):
    ...
    try:
        ...
        with anyio.CancelScope() as sc:
            async with HangupBridgeState.new(
                obj.ari, name=obj.cfg.asterisk.bridge_name
            ) as br:
                obj.bridge.br = br
                obj.bridge.scope = sc
                task_status.started()
...
async def monitor_redirect(obj):
    ...
    async with obj.dkv.watch(
        obj.cfg.replace.tree, min_depth=1, max_depth=1, fetch=True
    ) as mon:
...
async def _call(obj, n, dest, cid):
    ...
    with anyio.CancelScope() as sc:
        obj.calls.data[n] = sc
        try:
            ch = await obj.bridge.br.dial(
                endpoint=dest, State=partial(CalleeState, obj, n), callerId=cid
            )
...
async def door_call(obj, audio=True, opener=None):
    ...
    async with with_bridge(obj) as br:
        try:
            r = await br.dial(
                endpoint=obj.cfg.door.phone,
                State=partial(DoorState, obj, audio=audio, opener=opener),
            )
            return r
    ...
    try:
        ...
        async with with_bridge(obj) as br:
            ...
            try:
                ...
                return
            obj.log.info(
                "from door: %s: Connected to door (%r)", name, br.bridge.channels
            )
            obj.calls.data = []
            ...
            if r.channel.bridge is None:
                ...
                for ch in chs:
                    ...
                    pp.append(
                        await ch.playWithId(
                            playbackId=playback_id, media="tone:stutter;tonezone=de"
                        )
                    )
                try:
...
async def main(ctx, verbose, quiet, cfg):
    ...
    obj.ari = await ctx.with_async_resource(
        asyncari.connect(ast.url, ast.app, ast.username, ast.password)
    )
